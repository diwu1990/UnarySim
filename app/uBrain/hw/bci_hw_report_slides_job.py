...
def bci_hw_report(cpu_dir="/home/diwu1990/Dropbox/Project/UnaryComputing/2022-uBrain/result/mi/log_cpu",
                    systolic_dir="/home/diwu1990/anaconda3/envs/UnarySim_env/lib/python3.10/site-packages/UnarySim/app/uBrain/hw/systolic_array/result/",
                    sc_ubrain_path="/home/diwu1990/anaconda3/envs/UnarySim_env/lib/python3.10/site-packages/UnarySim/app/uBrain/hw/data_extract/",
                    sc_ubrain_file="uBrain_resource.xlsx",
                    output_path="/home/diwu1990/anaconda3/envs/UnarySim_env/lib/python3.10/site-packages/UnarySim/app/uBrain/hw/result_fig/"):
    ...
    x_axis = ["CPU", "Systolic", "uBrain"]
    fig, ax = plt.subplots(figsize=(fig_w, fig_h))
    x_idx = np.arange(len(x_axis))
    width = 0.2
    # area_list_2d = [area_cpu, area_sys, area_sto_bp, area_ubr_bp]
    area_list_2d = [area_cpu, area_sys, area_ubr_bp]
    area_list_2d = np.array(area_list_2d).T

    ax.bar(x_idx-width, area_list_2d[0], width, hatch = None, alpha=0.99, color=yellow, label='Sense')
    ax.bar(x_axis, area_list_2d[1], width, hatch = None, alpha=0.99, color=blue, label='Store')
    ax.bar(x_idx+width, area_list_2d[2], width, hatch = None, alpha=0.99, color=green, label='Compute')
    plt.yscale("log")
    ...
    plt.savefig(output_path+"/Area_total_slides_job.pdf", bbox_inches='tight', dpi=my_dpi, pad_inches=0.02)
    ...
    line_width = 1
    marker_size = 2
    ax.plot(np.array(runtime_cpu[0:10]) + runtime_window, np.sum(power_cpu[0:10], axis=1), "-^", alpha=0.99, color=yellow, label='CPU', lw=line_width, ms=marker_size)
    ax.plot(np.array(runtime_sys) + runtime_window, np.sum(power_sys, axis=1), "-P", alpha=0.99, color=blue, label='Systolic', lw=line_width, ms=marker_size)
    # ax.plot(runtime_unary, np.sum(power_sto_bp), ">", alpha=0.99, color=blue, label='SC', lw=line_width, ms=marker_size)
    ax.plot(runtime_unary, np.sum(power_ubr_bp), "*", alpha=0.99, color=red, label='uBrain', lw=line_width, ms=marker_size*2)
    plt.yscale("log")
    ...
    plt.savefig(output_path+"/Power_total_slides_job.pdf", bbox_inches='tight', dpi=my_dpi, pad_inches=0.02)
    ...
    plt.savefig(output_path+"/Power_onchip_slides_job.pdf", bbox_inches='tight', dpi=my_dpi, pad_inches=0.02)
    ...
    plt.savefig(output_path+"/Area_layerwise_slides_job.pdf", bbox_inches='tight', dpi=my_dpi, pad_inches=0.02)
    ...
    line_labels = ["BUF", "RNG", "CNT", "CMP", "PC", "REST"]
    fig.legend([l1, l2, l3, l4, l5, l6], line_labels, loc="upper center", ncol=6, frameon=True)
    plt.savefig(output_path+"/Power_layerwise_slides_job.pdf", bbox_inches='tight', dpi=my_dpi, pad_inches=0.02)
