...
class BasicBlock(nn.Module):
    ...
    def __init__(self, in_planes, planes, stride=1, cycle=None):
        ...
        if stride != 1 or in_planes != self.expansion*planes:
            self.shortcut = nn.Sequential(
                HUBConv2d(in_planes, self.expansion*planes,
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, in_planes, planes, stride=1, cycle=None):
        ...
        self.conv1 = HUBConv2d(in_planes, planes, kernel_size=1, bias=False, cycle=cycle)
        self.bn1 = nn.BatchNorm2d(planes)
        self.conv2 = HUBConv2d(planes, planes, kernel_size=3,
                               stride=stride, padding=1, bias=False, cycle=cycle)
        self.bn2 = nn.BatchNorm2d(planes)
        self.conv3 = HUBConv2d(planes, self.expansion *
...
class ResNet(nn.Module):
    def __init__(self, block, num_blocks, num_classes=10, cycle=None):
        ...
        self.conv1 = HUBConv2d(3, 64, kernel_size=3,
                               stride=1, padding=1, bias=False, cycle=cycle)
        ...
        self.linear = HUBLinear(512*block.expansion, num_classes, cycle=cycle)
