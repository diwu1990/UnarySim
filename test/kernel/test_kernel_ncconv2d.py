...
from UnarySim.kernel import FSULinearNC, FSUConv2dNC
...
def test_ncconv2d(epochs=1):
    ...
    class VGG5NC(torch.nn.Module):
        def __init__(self,time_step, dataset, hwcfg):
            super(VGG5NC, self).__init__()
            ...
            self.conv1 = FSUConv2dNC(input_dim, 64, kernel_size=3, padding=1, bias=False, hwcfg=hwcfg)
            self.pool1 = nn.MaxPool2d(kernel_size=2)
            self.conv2 = FSUConv2dNC(64, 128, kernel_size=3, padding=1, bias=False, hwcfg=hwcfg)
            self.conv3 = FSUConv2dNC(128, 128, kernel_size=3, padding=1, bias=False, hwcfg=hwcfg)
            self.conv4 = FSUConv2dNC(128, 256, kernel_size=3, padding=1, bias=False, hwcfg=hwcfg)
            self.pool2 = nn.MaxPool2d(kernel_size=2)
            self.fc_o5 = FSULinearNC(256*pre_linear_dim*pre_linear_dim, 10, bias=False, hwcfg=hwcfg)
    ...
    model = VGG5NC(time_step, dataset, hwcfg).to(device)
