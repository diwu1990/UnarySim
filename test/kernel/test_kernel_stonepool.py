...
from UnarySim.kernel.pool import FSUAvgPool2dStoNe
...
def test_stonepool():
    ...
    for mode in modes:
        ...
        dut_pool = FSUAvgPool2dStoNe(2, hwcfg=hwcfg).to(device)
        ...
        with torch.no_grad():
            ...
            for i in range(2**bitwidth):             # unary cycle count 2^n for n-bit binary data
                ...
                oVecU, _, u = dut_pool(iBS, u_prev[-1])                 # computing kernel, e.g., multiplication
                u_prev.append(u)
