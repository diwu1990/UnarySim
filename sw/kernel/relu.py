...
class UnaryReLU(torch.nn.Module):
    ...
    def UnaryReLU_forward(self, input):
        ...
        output = 1 - torch.ge(self.acc, self.buf_half).type(torch.int8) * (1 - input)
        self.acc.data = self.acc.add(input.mul(2).sub(1).type(torch.long)).clamp(0, self.buf_max.item())
        return output
